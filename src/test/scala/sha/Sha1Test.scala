...
class Sha01Test extends AnyFreeSpec with ChiselScalatestTester {
  "compute the correct hash value on a single char string" in {
    ...
    test(Sha1(params, len)) { c =>
      val expected =
        byteArrayToString(JAVA_SHA_1.digest("abc".getBytes("ASCII")))
      ...
      c.io.out.bits.expect(expected.U(160.W)) // Expected hash value in hex
    }
  }
  ...
  "compute the correct hash value for a simple message" in {
    ...
    test(new Md5(params, messageBitLength(BASE_TEST_STRING)))
